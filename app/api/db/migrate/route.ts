...
import { getAuthUser, getSupabaseServer, habitToDb, reminderToDb, taskToDb, projectToDb, personToDb } from '@/lib/db'
...
export async function POST(req: NextRequest) {
  ...
  await Promise.all([
    ...
    sb.from('user_settings').upsert({ user_id: userId, auto_remind: data.autoRemind ?? true, web_mode: data.webMode ?? false }),
  ])
